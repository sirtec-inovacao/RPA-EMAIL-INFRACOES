...
from src.ponto_mais.reports.types.records import columns
from src.ponto_mais.reports.period import getPeriod

def reports_records(operation: operations.Operations, data_in, data_fi, driver):
    getPeriod.writeDate("audit")
    try:
        print("INICIANDO DOWNLOAD DO RELATÓRIO DE REGISTRO DE PONTO. OPERAÇÃO: " + operation.operation)
        ...
        element = driver.find_element(By.XPATH, "/html/body/ng-dropdown-panel/div[2]/div[2]/div[20]/div/div/div[2]/span")
        driver.execute_script("arguments[0].scrollIntoView(true);", element)
        element.click()

    except: 
        print("Ocorreu algum erro durante a filtragem em records. (Pode não ser nada e reiniciar pode resolver)")

    modal.close(driver)
    columns.columns(driver)
    download_xlsx.download(driver, "records", operation.operation)
    logout(driver)
